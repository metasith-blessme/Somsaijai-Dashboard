...
for (const row of rows.slice(HEADER_ROWS)) {
  ...
  const key = `${month}|${cat === 'Salary' ? 'Salary' : cat === 'Rental' ? 'Rental' : 'Other'}`;
  existing[key] = (existing[key] || 0) + (Number(row[AMOUNT]) || 0);
}

// Shop rent and warehouse rent are both category Rental but must land on different P&L rows,
// so they are topped up separately and the description decides which row they hit.
// Anything already in the ledger for Q1 counts against shop rent first.
for (const [month, v] of Object.entries(SRC._sheet_fixed_costs_SGA)) {
  if (month.startsWith('_') || !MONTH_END[month]) continue;
  const items = [
    { category: 'Rental', label: `ค่าเช่าร้าน ${month}`, want: v.rent, have: existing[`${month}|Rental`] || 0 },
    { category: 'Rental', label: `ค่าเช่าคลัง ${month}`, want: v.warehouse, have: 0 },
  ...
  // The existing Q1 Rental rows are warehouse charges (12,324 / 12,000), so credit them there.
  const rentalHave = existing[`${month}|Rental`] || 0;
  items[0].have = 0;
  items[1].have = rentalHave;
